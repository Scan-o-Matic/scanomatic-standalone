...
def get_distance_matrix(measure: np.ndarray = gaussian(7, 1)) -> np.ndarray:
    """
...
def get_best_reference_for_experiments(
    ...
    def most_stable(curves, reference_weights, reference_filter):
        ...
        for i, pos in enumerate(positions):
            ...
            for other in positions[(positions != pos).any(axis=1)]:
                try:
                    sums[i] += (
                        np.abs(curves[pos] - curves[tuple(other)]).sum()
                        * reference_weights[pos]
                    )
    ...
    if reference_position_filter is None:
        reference_position_filter = get_reference_position_filter(
            plate.shape[:2],
        )
    ...
    if scale_references:
        ravel_references = np.ma.masked_invalid(
            plate[reference_position_filter].reshape(
        ...
        scale_vector = (
            ravel_experiments.mean(axis=0) / ravel_references.mean(axis=0)
        )

    for x, y in itertools.product(list(range(dim_x)), list(range(dim_y))):
        ...
        local_reference_filter = reference_position_filter[
            lower_x: upper_x,
            lower_y: upper_y
        ]
        local_reference_curves = plate[lower_x: upper_x, lower_y: upper_y]
        local_weights = distance_matrix[
            0 if x > offset else offset - x:
            min(distance_matrix_size, dim_x - (x - offset)),
            0 if y > offset else offset - y:
            min(distance_matrix_size, dim_y - (y - offset))
        ]

        reference_plate[x, y] = most_stable(
            local_reference_curves,
            local_weights,
            local_reference_filter,
        )
