...
def get_derivative(curve_strided, times_strided):
    ...
    for times, value_segment, filt in zip(
        times_strided,
        log2_strided_curve,
        filters,
    ):
...
def get_preprocessed_data_for_phenotypes(
    curve,
    curve_strided,
    flat_times,
    times_strided,
...
def curve_monotonicity(
    ...
    **kwargs,
    ...
    return (
        np.diff(
            curve_smooth_growth_data[
                curve_smooth_growth_data.mask == np.False_
            ]
        ) > 0
...
def get_chapman_richards_4parameter_extended_curve(
    x_data: np.ndarray,
    b0: float,
    b1: float,
    b2: float,
    ...
    Args:
        ...
        b0: The first parameter. To ensure that it stays within
            the allowed bounds b0 > 0, the input b0 is
            transposed using ``np.power(np.e, b0)``.

        b1: The second parameter. The bounds are
        ...
        b2: The third parameter, has same bounds and scaling as
            the first

        b3: The fourth parameter, has bounds 0 < b3 < 1, thus
            scaling is done with::

            ``np.power(np.e, b3) / (np.power(np.e, b3) + 1)``

        d:  Any real number, used as the offset of the log2_curve,
            no transformation applied.

    Returns:
        An array of matching size as X with the
        Chapman-Ritchards extended log2_curve for the parameter set.
...
def get_fit_r_square(
    ...
    try:
        p = leastsq(
            get_chapman_richards_residuals,
            p0,
            args=(x_data, y_data),
        )[0]
    except TypeError:
...
def get_chapman_richards_residuals(
...
def generation_time(derivative_values_log2, index: int, **kwargs) -> float:
    if index < 0:
        ...
        _logger.warning("Faulty index {0} for GT (max {1})".format(
            index,
            derivative_values_log2.size - 1,
        ))
        return np.nan
...
def generation_time_error(derivative_errors, index: int, **kwargs) -> float:
    if index < 0:
        ...
        _logger.warning("Faulty index {0} for GT error (max {1})".format(
            index,
            derivative_errors.size - 1,
        ))
        return np.nan
...
def generation_time_when(
    ...
    **kwargs,
    ...
    if pos < 0:
        ...
        _logger.warning("Faulty index {0} for GT when (max {1})".format(
            pos,
            flat_times.size - 1,
        ))
        return np.nan
...
def growth_lag(
    ...
    **kwargs,
    ...
    growth_delta = (
        np.log2(population_size_at_generation_time(index=index, **kwargs))
        - np.log2(curve_baseline(**kwargs))
    )

    if growth_delta > 0:
        return np.interp(
            max(0.0, growth_delta / derivative_values_log2[index]),
            np.arange(flat_times.size),
            flat_times,
        )
    return np.nan
...
def _get_generation_time_index(log2_masked_derivative_data, rank):
    finites = (
        log2_masked_derivative_data.size
        - log2_masked_derivative_data.mask.sum()
    )
...
class Phenotypes(Enum):
    ...
    Each attribute can be called to perform that calculation given that
    ...
    def __call__(self, **kwargs):
        ...
        if self is Phenotypes.InitialValue:
            ...
            return population_size_at_generation_time(
                index=_get_generation_time_index(
                    kwargs['derivative_values_log2'],
                    0,
                ),
