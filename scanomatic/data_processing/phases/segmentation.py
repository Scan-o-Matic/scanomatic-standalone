...
def is_detected_non_linear(phase_type: CurvePhases) -> bool:
    return phase_type in (
...
def is_detected_linear(phase_type: CurvePhases) -> bool:
    return phase_type in (
        CurvePhases.Flat,
        CurvePhases.Collapse,
        CurvePhases.Impulse,
    )


def is_undetermined(phase_type: CurvePhases) -> bool:
    return phase_type in (
        CurvePhases.Undetermined,
        CurvePhases.UndeterminedNonFlat,
        CurvePhases.UndeterminedNonLinear,
...
def segment(
    ...
    # IMPORTANT, should be before having set flat so that there are no edge
    # conditions.
    extensions, _ = get_linear_non_flat_extension_per_position(
        segmentation_model,
        thresholds,
    )
    ...
    for filt in _get_candidate_segment(
        segmentation_model.phases,
        test_value=CurvePhases.UndeterminedNonLinear.value,
    ):
...
def _fill_undefined_gaps(phases):
    ...
    for loc in undefined:
        ...
        if loc == 0:
            ...
            phases[loc - 1] == phases[loc + 1]
            and phases[loc + 1] != CurvePhases.Undetermined.value
            and phases[loc + 1] not in (
                CurvePhases.Impulse.value,
                CurvePhases.Collapse.value,
            )
        ):
            phases[loc] = phases[loc + 1]
...
def get_tangent_proximity(model, loc, thresholds):
    ...
    # Find all candidates
    return (
        np.abs(model.log2_curve - tangent)
        < np.abs(thresholds[Thresholds.LinearModelExtension] * loc_slope)
    ).filled(False)
...
def _validate_linear_non_flat_phase(model, elected, phase, thresholds):
    ...
    if model.offset:
        ...
        if (
            model.log2_curve[model.offset: -model.offset][right]
            ...
            #    np.abs(model.log2_curve[left] - model.log2_curve[right]),
            #    None,
            #    model.plate,
            #    model.pos,
            # ))
        ...
        model.log2_curve[right] - model.log2_curve[left]
    ) * (
        -1 if phase is CurvePhases.Collapse else 1
    ) < thresholds[Thresholds.NonFlatLinearMinimumYield]:
        # print("***Failed phase ({2}, {3}): {0:.2f}".format(
    ...
    print("*Good phase ({2}, {3}): {0:.2f}, {1:.2f}".format(
        model.log2_curve[left],
        model.log2_curve[right],
        model.plate,
        model.pos,
    ))
    """
...
def classifier_nonflat_linear(model, thresholds, filt):
    ...
    if model.dydt[filt].any() in (np.ma.masked, False):
        ...
        return (
            CurvePhases.Undetermined,
            np.zeros_like(filt).astype(bool),
            False,
        )
    ...
    filt = (
        filt
        | (model.phases == CurvePhases.Undetermined.value)
        | (model.phases == CurvePhases.UndeterminedNonFlat.value)
    )
    ...
    if n_found == 0 or not candidates[loc]:
        ...
        return (
            CurvePhases.Undetermined,
            np.zeros_like(candidates).astype(bool),
            False,
        )
    ...
    if not _validate_linear_non_flat_phase(model, elected, phase, thresholds):
        if elected.sum() < thresholds[Thresholds.NonFlatLinearMergeLengthMax]:
            ...
            flanks = set(
                f for f in _get_flanks(model.phases, elected)
                if f is not None
            )
            if len(flanks) == 1:
                # We must ensure that we don't return phase to more initial
                # states of indeterminate.
                flank = flanks.pop()
                if flank not in (
                    CurvePhases.Undetermined.value,
                    CurvePhases.UndeterminedNonFlat.value,
                ):
                    if model.pos == (8, 3):
                        print(
                            "***Special condition setting elected on curve ({2}, {3}) to flank {0} {1}".format(  # noqa: E501
                                flank,
                                elected,
                                model.plate,
                                model.pos,
...
def _set_nonflat_linear_segment(model, thresholds):
    ...
    # TODO: Note that it can cause an issue if log2_curve is very wack, could
    # merge two segments that shouldn't be Probably extremely unlikely
...
def classifier_nonlinear(
    ...
    phase = classify_non_linear_segment_type(
        model,
        thresholds,
        filt,
        test_edge,
    ...
    # Determine which operators to be used for first (op1) and second (op2)
    # derivative signs
    if (
        phase is CurvePhases.GrowthAcceleration
        or phase is CurvePhases.GrowthRetardation
    ):
        op1 = operator.ge
    else:
        op1 = operator.le
    ...
    if (
        phase is CurvePhases.GrowthAcceleration
        or phase is CurvePhases.CollapseRetardation
    ):
        op2 = operator.ge
    else:
        op2 = operator.le
    ...
    if label_count:
        candidates = candidates == (
            1 if test_edge is PhaseEdge.Left else label_count
        )
...
def classify_non_linear_segment_type(
    model: SegmentationModel,
    thresholds: ThresholdsDict,
    filt: np.ndarray,
    test_edge,
    ...
    if any((
        ...
        for v in (
            filt.sum(),
            (~model.dydt[filt].mask).sum(),
            (~model.d2yd2t[filt].mask).sum(),
        )
    )):
    ...
    if test_edge is PhaseEdge.Left:
        for test_length in range(
            ...
            phase = _classify_non_linear_segment_type(
                dydt_section,
                d2yd2t_section,
                thresholds,
            )
...
def _set_nonlinear_phase_type(
    ...
    phase, candidates = classifier_nonlinear(
        model,
        thresholds,
        filt,
        test_edge,
    ...
    if (
        phase is CurvePhases.Undetermined
        or candidates.sum() < thresholds[Thresholds.PhaseMinimumLength]
    ):
        return CurvePhases.Undetermined
...
def _classify_non_linear_segment_type(
    ...
    if (
        d2yd2t_section.size == 0
        or d2yd2t_section.sum() == 0
        or dydt_section.sum() == 0
    ):
        return CurvePhases.Undetermined
...
def set_nonflat_linearity_segments(model, extension_lengths, thresholds):
    ...
    # print("{0}, {1}: Candidates ({3}) {2}".format(
    #     model.plate,
    #     model.pos,
    #     labeled_candidates,
    #     n_candidates,
    # ))
    labeled_candidates, n_candidates = get_barad_dur_towers(
        extension_lengths,
        filt,
        thresholds,
    )
    ...
    for candidate in range(1, n_candidates + 1):
        ...
        while cur_filt.any():
            ...
            #     positions[i + 1] + 1,
            # ))
            phase, elected, valid = classifier_nonflat_linear(
                model,
                thresholds,
                cur_filt,
            )
            if valid:
                ...
                elected_padding = (
                    ext_elected.astype(int) - elected.astype(int)
                ) > 0
                ...
                # print("***Invalid segment {1}, filt was {0}".format(
                #     cur_filt.astype(int),
                #     elected.astype(int),
                # ))
                cur_filt = cur_filt & (
