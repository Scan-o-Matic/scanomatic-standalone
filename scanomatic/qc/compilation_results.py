    ...
    def wrapped(*args, **kwargs):
        ...
        if len(args) > 0:
            ...
            if isinstance(args[0], str):
                ...
                args = list(args)
                args[0] = CompileImageAnalysisFactory.get_serializer().load(args[0])
...
def get_grayscale_outlier_images(
    ...
    return [
        (i if only_image_indices else image)
        for i, image in enumerate(project_compilation)
...
def plot_grayscale_histogram(
    ...
    data = np.array(data)
    if mark_outliers:
        outliers = get_grayscale_outlier_images(
            project_compilation,
            max_distance,
        ) if mark_outliers else []
    else:
        outliers = None
    f = plt.figure()
    f.clf()
    ...
    ax.set_title(
        "Grayscale segment measured values as colors" +
        (
            (" (arrows, outliers)" if outliers else " (no outliers)")
            if mark_outliers else ""
        )
    )
    if outliers:
...
def animate_marker_positions(
    ...
    save_target="marker_positions.avi",
    ...
    paths = [
        image.image.path if os.path.isfile(image.image.path)
        else os.path.basename(image.image.path)
        for image in project_compilation
    ]
        ...
        for index in range(positions.shape[0]):
            ...
            if data[index] is None:
                ...
                for im_index, im in enumerate(images):
                    im_slice = make_cutout(
                        image,
                        *positions[index, :, im_index],
                    )
...
def get_irregular_intervals_from_file_names(directory, max_deviation=0.05):
    images = [
        float(_time_pattern.findall(f)[-1])
        for f in sorted(glob.glob(os.path.join(directory, "*.tiff")))
        ...
        for idx0, series in enumerate(im):
            for idx1, value in enumerate(series):
                ax.annotate(
                    value,
                    (idx0, idx1),
                    color=cm(0.9) if value < scans/2.0 else cm(0.1),
                    ha='center',
                    va='center',
                )
...
def get_positional_marker_outlier_images(
    ...
    return (
        irregulars if only_image_indices
        else tuple(project_compilation[i] for i in irregulars)
    )
...
def _get_marker_sorted_data(project_compilation):
    ...
    return np.array([d[:, s] for d, s in zip(data, sortorder)])
...
def get_images_with_irregularities(
    ...
    data = set(
        get_grayscale_outlier_images(
            project_compilation,
            only_image_indices=only_image_indices,
        ),
    ).union(
        get_positional_marker_outlier_images(
            project_compilation,
            only_image_indices=only_image_indices
        ),
    )
