...
def animate_colony_growth(
    save_target,
    position,
    analysis_folder,
    fps=12,
    project_compilation=None,
    fig=None,
    cmap=plt.cm.gray,
    colony_title=None,
    positioning="one-time",
):

    _logger.info("Loading colony images")
    times, images, _ = load_colony_images_for_animation(
        analysis_folder,
        position,
    ...
    _, curve_times, polygon = plot_growth_curve(
        analysis_folder,
        position,
        curve_ax,
    )
    ...
    def _plotter():
        ...
        for i, time in enumerate(times):
            im_ax.set_title(
                colony_title if colony_title is not None
                else "Pos {0}, (t={1:.1f}h)".format(position, time / 3600.)
            )
...
def detection_files(
    ...
    pattern = os.path.join(
        source_location,
        "grid_cell_*_{0}_{1}_{2}".format(*data_pos) + suffix,
    )
...
def animate_blob_detection(
    ...
    curve_ax, curve_times, polygon = plot_growth_curve(
        analysis_folder,
        position,
        fig.axes[-1],
    )
        ...
        for idx, index in enumerate(image_indices):
            ...
            image_ax.set_title(
                "Image (t={0:.1f}h)".format(
                    image_indices[index] if interval is None
                    else image_indices[index] * interval
                )
            )

            for j, ending in enumerate((
                '.background.filter.npy',
                '.blob.filter.npy',
...
def animate_example_curves(
    ...
    **kwargs,
    ...
    if isinstance(growth_data, bytes):
    ...
    def _plotter():
        ...
        while elapsed < duration:
            ...
            pos = tuple(
                np.random.randint(p_shape)
                for p_shape in data[0][plate].shape[:2]
            )
            if ax_title is None:
