...
from scanomatic.data_processing.phases.features import (
    ...
    get_phase_assignment_frequencies,
...
def get_position_phenotypes(phenotypes, plate, position_selection=None):
    return {
        phenotype.name: phenotypes.get_phenotype(
            phenotype
        )[plate][position_selection] for phenotype in Phenotypes
    }
...
def get_linkage_matrix(
    ...
    if aligned_phases.ndim > 2:
        aligned_phases = aligned_phases.reshape(
            (np.prod(aligned_phases.shape[:-1]), ) + aligned_phases.shape[-1:]
        )
    ...
    if hasattr(aligned_phases, 'mask') and aligned_phases.mask.any():
        ...
        if nan_to_mean:
            for i in range(aligned_phases.shape[1]):
                if mask[:, i].any():
                    aligned_phases[:, i][mask[:, i]] = (
                        aligned_phases[:, i][mask[:, i] == np.False_].mean()
                    )
...
def plot_phase_correlation_dendrogram(
    ...
    phase_key[:, 0] = tuple(
        chain(*((i,) * len(v[1]) for i, v in enumerate(phase_labels))),
    )
    phase_key[:, 1] = tuple(
        chain(*((v[0].value,) * len(v[1]) for i, v in enumerate(phase_labels))),
    )
    phase_key[:, 2] = tuple(
        chain(*((v2.value for v2 in v[1]) for i, v in enumerate(phase_labels))),
    )
    ...
    dend_ax.set_yticks([])
    dend_ax.set_title(
        "Similarity ({0}, {1}) of phases".format(
            clust_method,
            clust_metric,
        ),
        size='small'
    ...
    f.legend(
        ...
        bbox_to_anchor=(
            l_x,
            ax_low + l0_height + l1_height + l_spacer * 2,
            l_width,
            l2_height,
        ),
        bbox_transform=f.transFigure,
...
def plot_plate_phase_assigment_frequencies(phenotypes, plate):
    ...
    var = np.ma.masked_invalid(
        np.log2(phenotypes.smooth_growth_data[plate]).reshape(a * b, c),
    ).var(axis=0)
    ...
    tax.plot(
        ...
        color='k',
        lw=2.5,
        label="Population size variance",
    )
    ...
    ax2.pie(
        var_decomp,
        colors=tuple(
            PHASE_PLOTTING_COLORS[CurvePhases(idx)]
            for idx in range(var_decomp.size)
        ),
        labels=tuple(
            CurvePhases(idx).name.replace("Growth", "G ").replace(
                "Collapse",
                "C",
            )
...
def plot_plate_phase_pop_size_variance_decomp(
    ...
    phase_var = np.zeros(
        (
            max(phase.value for phase in CurvePhases) + 1,
            log2_smooth.shape[-1]
        ),
    )
    ...
    for idx, data in enumerate(phase_var):
        ...
        cum_sum[...] = [
            c if np.ma.is_masked(m) else m for c, m in zip(cum_sum, vals)
        ]
    ...
    ax.set_xlabel("Time (h)")
    ax.set_title(
        "Plate {0} phase variance decomposition by phase assignment".format(
            plate + 1,
        )
    )
    tax.set_ylabel("Log2 Population Size Variance")
...
@_validate_input
def plot_all_curves_and_smoothing(
    phenotyper_object,
    id_plate,
    f=None,
    ...
    plate_shape = (
        plot_to_pos[0] - plot_from_pos[0],
        plot_to_pos[1] - plot_from_pos[1]
    )
    times = phenotyper_object.times
    if smoothing_color is None:
        smoothing_color = PHASE_PLOTTING_COLORS['smooth']

    for i, (id1, id2) in enumerate(product(
        list(range(plot_from_pos[0], plot_to_pos[0])),
        list(range(plot_from_pos[1], plot_to_pos[1])),
    )):
        ...
        if plot_raw:
            ...
            ax.semilogy(
                ...
                ms=2,
                label="Raw data",
                color=PHASE_PLOTTING_COLORS['raw'],
            )
        curve = plate[id1, id2]
        ax.semilogy(
            times,
...
def plot_phases_all_curves(
    ...
    for i, (id1, id2) in enumerate(product(
        ...
        plot_phases_from_data(
            ...
            deriv=deriv,
            plot_legend=False,
            set_labels=i == 0 and not hide_all_legends,
            layout=False,
        )
...
def plot_phases_legend(f=None, colors=None):
    ...
    for phase in CurvePhases:
        ...
        legend.append(mpatches.Patch(
            color=colors[phase],
            label=re.sub(r"([a-z])([A-Z])", r'\1 \2', phase.name),
        ))
...
def plot_barad_dur_plot(
    ...
    ext_val, _ = get_linear_non_flat_extension_per_position(
        model,
        DEFAULT_THRESHOLDS,
    )
    ...
    towers, n_towers = get_barad_dur_towers(
        ext_val,
        non_flat_filt,
        DEFAULT_THRESHOLDS,
    )
    ...
    ax.set_xlabel("Time [h]")
    ax.set_ylabel(
        "Length of local linearity in number of indices"
    )
    ax.set_title(
        "Barad-dûr plot of plate {0} pos {1}".format(
            model.plate,
            model.pos,
        ),
    )
    ...
    for i in range(1, n_flats + 1):
        line_out = ax.plot(
            phenotyper_object.times[flat_parts == i],
            ext_val[flat_parts == i],
            ':',
            color='r',
            label="Flat segment"
        )[0]
    ...
    for i in range(1, n_nonflats + 1):
        line_in = ax.plot(
            phenotyper_object.times[non_flat_parts == i],
            ext_val[non_flat_parts == i],
            '-',
            color='k',
            label="Non-flat segment",
        )[0]
    ...
    if plot_final_phases:
        non_flat_linear = (
            phenotyper_object.get_curve_phases(
                model.plate,
                model.pos[0],
                model.pos[1],
            ) == 4
        ...
        legend_labels.append(
            line_phases.get_label() if line_phases
            else "Linear Phase(s) (missing)"
        )
    ...
    if n_towers:
        ...
        for i in range(1, n_towers + 1):
            tower_line = ax.fill_between(
                phenotyper_object.times[towers == i],
                ext_val[towers == i],
                0,
                color='k',
                edgecolor='k',
    ...
    legend_labels.append(
        tower_line.get_label() if tower_line else "Tower(s) (missing)",
    )
...
def plot_curve_and_derivatives(
    ...
    **kwargs,
    ...
    return plot_curve_and_derivatives_from_model(
        model,
        thresholds,
        show_thresholds,
        ax=ax,
...
def plot_curve_and_derivatives_from_model(
    ...
    for val, color in zip((-1, 0, 1), ('r', 'k', 'g')):
        filt = model.d2yd2t_signs != val
        ax.plot(
            times,
            np.choose(filt, (model.d2yd2t, nan_arr)),
            lw=2.5,
            color=color,
        )
    ...
    if show_thresholds:
        ...
        t1 = model.d2yd2t.std() * thresholds[
            Thresholds.SecondDerivativeSigmaAsNotZero
        ]
    ...
    for val, color in zip((-1, 0, 1), ('r', 'k', 'g')):
        ...
        ax2.plot(
            times,
            np.choose(filt, (model.dydt, nan_arr)),
            lw=2.5,
            color=color
        )
    ...
    ax.set_xlabel("Hours")
    ax.set_title(
        "Curve {0} and its derviatives".format((model.plate, model.pos)),
    )
    """
    legend = ax.legend(
...
def load_phenotype_results_into_plates(
    ...
    for plateIndex in data.Plate.unique():
        ...
        for _, dataRow in plate.iterrows():
            plates[plateIndex][dataRow.Row, dataRow.Column] = (
                dataRow[phenotype_header]
            )
...
def animate_plate_over_time(
    ...
    if truncate_value_encoding:
        ...
        vmax = masked_plate[
            argorder[np.round(argorder.size * (1 - fraction))]
        ]
...
def plot_phases(
    ...
    ax.set_title(
        "Curve phases for plate {0}, position ({1}, {2})".format(
            plate,
            *position,
        )
    )
...
def plot_phases_from_data(
    ...
    for phase in CurvePhases:
        ...
        for id_label in range(1, label_count + 1):
            ...
            right = np.linspace(
                times[min(log2_curve.size - 1, right + 1)],
                times[right],
                3,
            )[1]
            span = ax.axvspan(
    ...
    if deriv is not None:
        ...
        if times.size > deriv.size:
            ...
            assert (
                delta % 2 == 0
            ), "The derivative to times offset is not multiple of 2"
            delta /= 2
...
def plot_phase_segmentation_in_steps(
    ...
    **kwargs,
    ...
    if 'f' in kwargs:
        ...
        f = plt.figure(
            "curve_{0}_{1}_{2}_segmentation_steps".format(plate, *position),
        )
