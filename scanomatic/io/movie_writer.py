...
class MovieWriter:
    def __init__(
        self,
        target,
        title: str = 'Animation Plotting',
        artist: str = 'Scan-o-Matic',
        comment: str = '',
        fps: int = 15,
        dpi: int = 100,
        fig=None,
        ...
        :param fig: The figure to be used (optional), it can also be inferred
        from the drawing function's signature or automatically created by the
        Write_Movie instance
        :return:

        # Usage

        The Write_Movie can either be used as decorator to a function (see
        movie_writer.demo) instantiated and manually passed the drawing
        function.

        The instances should be called with a drawing function as its primary
        argument, all other arguments are passed along to the drawing
        function.

        The drawing function needs to return an iterator (see
        movie_writer.demo for example)
        """
    ...
    def __call__(self, drawing_function):
        def wrapped(*args, **kwargs):
            ...
            with (
                ...
                print(
                    "Animation done! (Process took: {0:.2f}s)".format(
                        time.time() - start_time,
                    )
                )
