...
def get_segments_by_shape(
    ...
    for i, segment in enumerate(segments):
        s_d1, s_d2 = segment
        if (
            abs(s_d1.stop - s_d1.start) > bound_d1
            or abs(s_d2.stop - s_d2.start) > bound_d2
        ):
            ...
            if (
                is_almost_square(feature_slice)
                or is_almost_round(feature_slice, blob)
            ):
                out[segment][labeled_im[segment] == i + 1] = False
...
def is_almost_round(feature_slice, blob):
    ...
    return (
        abs(
            1
            - feature_slice.size * _INSET_CIRCLE_IN_SQUARE_FACTOR / blob.sum()
        )
        > _DEVIATION_FROM_CIRCLE_TOLERANCE
    )
...
def get_weights(votes, data, width=1.0):
    ...
    if width > 0:
        weights = (
            np.exp(
                -(
                    (x_votes - x_data.mean()) ** 2
                    / (width * x_data.std()) ** 2
                    + (y_votes - y_data.mean()) ** 2
                    / (width * y_data.std()) ** 2
                )
...
def replace_ideal_with_observed(ideal_grid, x_data, y_data, max_sq_dist):
    ...
    def _look_replace(array_view, filt):
        ...
        distances = (
            np.subtract.outer(ideal_xs, x_data) ** 2
            + np.subtract.outer(ideal_ys, y_data) ** 2
        )
        ...
        for i in range(where_pos[0].size):
            d1 = where_pos[0][i]
            d2 = where_pos[1][i]
            if filt[d1, d2]:
    ...
    def _push_ideal(array_view, current_ring):
        ...
        if (rings[0] - current_ring - 1) >= 0:
            distance_dim1_lower = (
                ideal_grid[0, rings[0] - current_ring - 1, :].mean()
                - array_view[0, 0, :].mean()
            )
            distance_dim1_upper = (
                ideal_grid[0, rings[0] + current_ring, :].mean()
                - array_view[0, -1, :].mean()
            )
            distance_dim1_lower_slice = np.s_[0, : rings[0] - current_ring, :]
            ideal_grid[distance_dim1_lower_slice][
                update_allowed[distance_dim1_lower_slice[1:]]
            ] -= distance_dim1_lower
            distance_dim1_upper_slice = np.s_[0, rings[0] + current_ring:, :]
            ideal_grid[distance_dim1_upper_slice][
                update_allowed[distance_dim1_upper_slice[1:]]
            ] -= distance_dim1_upper

        if (rings[1] - current_ring - 1) >= 0:
            distance_dim2_lower = (
                ideal_grid[1, :, rings[1] - current_ring - 1].mean()
                - array_view[1, :, 0].mean()
            )
            distance_dim2_upper = (
            ...
            ideal_grid[distance_dim2_lower_slice][
                update_allowed[distance_dim2_lower_slice[1:]]
            ] -= distance_dim2_lower
            distance_dim2_upper_slice = np.s_[1, :, rings[1] + current_ring:]
            ideal_grid[distance_dim2_upper_slice][
                update_allowed[distance_dim2_upper_slice[1:]]
            ] -= distance_dim2_upper

    for ring in range(int(rings.max())):
        if (rings[0] - ring - 1) > 0:
        ...
        update_filter = update_allowed[
            shape_dim1_lower: shape_dim1_upper,
            shape_dim2_lower: shape_dim2_upper
        ]
        section = ideal_grid[
            :,
            shape_dim1_lower: shape_dim1_upper,
...
def get_validated_grid(im, grid, delta_dim1, delta_dim2, adjusted_values):
    ...
    if (
        grid[delta_slice_dim1].min() < np.ceil(delta_dim1 * 0.5)
        or grid[delta_slice_dim2].min() < np.ceil(delta_dim2 * 0.5)
    ):
    ...
    if (
        grid[delta_slice_dim1].max() >= im.shape[0] + np.ceil(delta_dim1 * 0.5)
        or (
            grid[delta_slice_dim2].max()
            >= im.shape[1] + np.ceil(delta_dim2 * 0.5)
        )
    ):
...
def get_valid_parameters(
    ...
    print(
        '*** Returning center {0} and spacing {1}'.format(center, spacing),
    )
...
def get_grid(
    ...
    get_segments_by_size(
        im_filtered,
        min_size=40,
        max_size=expected_spacing[0] * expected_spacing[1],
        inplace=True,
    )
    ...
    if adjusted_values is False:
        ...
        if grid_correction is not None:
            center = tuple(
                ...
                zip(
                    center,
                    [i * j for i, j in zip(grid_correction, spacings)],
                )
            )
            adjusted_values = True
